...
public class AnswersFacadeREST extends AbstractFacade<Answers> {
    ...
    @Path("{id}")
    @Consumes({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public void edit(@PathParam("id") Long id, Answers entity) {
        super.edit(entity);
    }
    ...
    public Answers find(@PathParam("id") Long id) {
    ...
    @Path("/find/{id}")
    @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public List<Answers> findByQuestionId(@PathParam("id") String id) {
        Query query = em.createQuery("SELECT a FROM Answers a WHERE a.questionid.id = :id")
